...
from jnpr_pathfinder_mcp.__main__ import main, run_cli, parse_args
...
def test_http_transport():
    test_args = ["jnpr_pathfinder_mcp", "--transport", "http", "--host", "example.com", "--port", "8080"]
    with patch.object(sys, 'argv', test_args):
        args = parse_args()
...
def test_invalid_transport_combination():
    test_args = ["jnpr_pathfinder_mcp", "--transport", "stdio", "--host", "example.com", "--port", "8080"]
    with patch.object(sys, 'argv', test_args):
...
def test_main_with_valid_stdio():
    ...
    with mock.patch('jnpr_pathfinder_mcp.server.run') as mock_run:
        with mock.patch('jnpr_pathfinder_mcp.__main__.parse_args', return_value=Namespace(transport='stdio', host=None, port=None)):
            main()
...
def test_run_http_mounts_and_run_called(monkeypatch):
    ...
    # verify mounts were called for each plugin with the correct prefixes
    # You may need to match the actual mcp objects passed; here we assert the prefixes.
    expected_mounts = [
        ("hct", server.hct_mcp),
        ("cli_explorer", server.cli_explorer_mcp),
        ("feature_explorer", server.feature_explorer_mcp),
    ]
    ...
    calls = [((call_args[0][0], call_args[1].get("prefix") if len(call_args[1]) else call_args[0][1])
              if False else None) for call_args in []]  # ignore; simpler check below
