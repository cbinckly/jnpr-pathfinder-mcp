...
async def test_categories_requests_error_raises():
    """Test content and structure of workspace info command."""
    async with Client(mcp) as client:
        with mock.patch.object(jnpr_pathfinder_mcp.server.cli_explorer.requests, "get", side_effect=requests.exceptions.RequestException):
...
async def test_categories_request_not_ok():
    """Test content and structure of workspace info command."""
    async with Client(mcp) as client:
        with mock.patch.object(jnpr_pathfinder_mcp.server.cli_explorer.requests, "get", return_value=ResponseMock(False, "Failed.")):
            result = await client.call_tool("categories")
...
async def test_categories_request_empty_response():
    """Test content and structure of workspace info command."""
    async with Client(mcp) as client:
        with mock.patch.object(jnpr_pathfinder_mcp.server.cli_explorer.requests, "get", return_value=ResponseMock(True, "")):
            result = await client.call_tool("categories")
            assert not result.structured_content.get('success')
            assert result.structured_content.get('error')
...
async def test_component_supported_platforms():
    """Test content and structure of workspace info command."""
    async with Client(mcp) as client:
        result = await client.call_tool("component_supported_platforms", {"component_name": "OSFP-800G-AOC-7M"})
        assert result.structured_content.get('success')
        assert result.structured_content.get('response')
...
async def test_component_supported_models():
    """Test content and structure of workspace info command."""
    async with Client(mcp) as client:
        result = await client.call_tool("component_supported_models", {"component_name": "QFX10000-30C"})
        assert result.structured_content.get('success')
        assert result.structured_content.get('response')
...
async def test_platform_hardware_details_requests_error_raises():
    """Test content and structure of workspace info command."""
    async with Client(mcp) as client:
        with mock.patch.object(jnpr_pathfinder_mcp.server.cli_explorer.requests, "post", side_effect=requests.exceptions.RequestException):
            with pytest.raises(ToolError):
                result = await client.call_tool("platform_hardware_details", {"platform": "EX4000"})
...
async def test_platform_hardware_details_request_not_ok():
    """Test content and structure of workspace info command."""
    async with Client(mcp) as client:
        with mock.patch.object(jnpr_pathfinder_mcp.server.cli_explorer.requests, "post", return_value=ResponseMock(False, "Failed.")):
            result = await client.call_tool("platform_hardware_details", {"platform": "EX4000"})
            assert not result.structured_content.get('success')
            assert result.structured_content.get('error')
            assert not result.structured_content.get('response')
...
async def test_platform_hardware_details_request_empty_response():
    """Test content and structure of workspace info command."""
    async with Client(mcp) as client:
        with mock.patch.object(jnpr_pathfinder_mcp.server.cli_explorer.requests, "post", return_value=ResponseMock(True, "")):
            result = await client.call_tool("platform_hardware_details", {"platform": "EX4000"})
            assert not result.structured_content.get('success')
            assert result.structured_content.get('error')
            assert 'Empty response from API' in  result.structured_content.get('error')
